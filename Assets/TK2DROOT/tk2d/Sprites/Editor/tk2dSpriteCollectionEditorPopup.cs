...
public class tk2dSpriteCollectionEditorPopup : EditorWindow 
{
	...
	public void SetGeneratorAndSelectedSprite(tk2dSpriteCollection _gen, int _selectedSprite)
	{
		gen = _gen;
		currSprite = _selectedSprite;
	}
	...
	void DrawSpritePropertiesPanel(ref int currSprite, ref tk2dSpriteCollectionDefinition param)
	{
		...
		if (!param.fromSpriteSheet)
		{
			...
			//param.padAmount = EditorGUILayout.EnumPopup("Pad", param.pad);
		...
		if (gen.allowMultipleAtlases)
		{
			tk2dGuiUtility.InfoBox("NOTE: Dicing is not allowed when multiple atlas build is enabled.", tk2dGuiUtility.WarningLevel.Warning);
		}
	}
	...
	void DrawPreviewFoldout(tk2dSpriteCollectionDefinition param, Texture2D displayTexture)
	{
		...
		if (GUILayout.Button("Reset", GUILayout.MaxWidth(45.0f))) displayScale = 1.0f;
		EditorGUILayout.EndHorizontal();
		...
		tk2dGuiUtility.InfoBox("Mouse wheel - zoom\n" +
					  			  "Middle mouse drag - pan", tk2dGuiUtility.WarningLevel.Info);
		...
		if (rect.Contains(Event.current.mousePosition))
		{
			...
			{
				displayScale -= Event.current.delta.y * 0.01f;
				Event.current.Use();
				Repaint();
			}
		}
